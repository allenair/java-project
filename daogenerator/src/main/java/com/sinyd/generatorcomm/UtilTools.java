...
public class UtilTools {
	...
	public static List<String> getTablePkMap(String tabName, Connection conn)throws SQLException{
		...
		/*ResultSet rs2 = dbm.getColumns(null, UtilTools.getProVal("DB.schema.name"), "SYS_LAB", "ID");
		while(rs2.next()){
			ResultSetMetaData ddd = rs2.getMetaData();
			int len = ddd.getColumnCount();
			for(int i=0;i<len;i++){
				String column = ddd.getColumnName(i+1);
				System.out.println(column+"----"+rs2.getString(column));
			}
			System.out.println(rs2.getString("TYPE_NAME"));
			System.out.println(rs2.getString("DATA_TYPE"));
		}*/
	...
	public static boolean isAutoincrement(String tabName,
			List<Map<String, String>> pkColArr, Connection conn) throws Exception {

		DatabaseMetaData dbm = conn.getMetaData();
		for(Map<String, String> map : pkColArr){
			ResultSet rs = dbm.getColumns(null, UtilTools.getProVal("DB.schema.name"), tabName, map.get("columnName"));
			while(rs.next()){
				String type_name = rs.getString("TYPE_NAME");
				String DATA_TYPE = rs.getString("DATA_TYPE");
				//System.out.println(type_name);
				//System.out.println(DATA_TYPE);
				if(DATA_TYPE.equals("4") && type_name.toLowerCase().indexOf("identity")>0){
					rs.close();
					rs = null;
					return true;
				}
	...
	public static boolean isAutoincrement(String tabName,
			String columnName, Connection conn) throws Exception {
		
		DatabaseMetaData dbm = conn.getMetaData();
		ResultSet rs = dbm.getColumns(null, UtilTools.getProVal("DB.schema.name"), tabName, columnName);
		while(rs.next()){
			String type_name = rs.getString("TYPE_NAME");
			String DATA_TYPE = rs.getString("DATA_TYPE");
			if(DATA_TYPE.equals("4") && type_name.toLowerCase().indexOf("identity")>0){
